...
const links = [
  {
    ...
    description:
      "Cartas contempladas e uma oferta por semana para comprar ou lucrar com consórcio.",
...
function Index() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-background">
      <div
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-96 bg-[radial-gradient(circle_at_70%_0%,var(--color-primary),transparent_62%)] opacity-45"
      />
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col px-5 pb-10 pt-3 sm:px-8 sm:pt-8 lg:grid lg:grid-cols-[1fr_1.1fr] lg:items-center lg:gap-14 lg:py-12">
        <section className="animate-entrance flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <div className="relative h-72 w-full max-w-sm overflow-hidden rounded-md border border-primary/25 bg-card sm:h-80 lg:h-[25rem]">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background via-background/65 to-transparent px-5 pb-5 pt-20 text-left">
              <p className="text-xs font-extrabold uppercase text-brand-bright">
        ...
        <section className="mt-8 lg:mt-0">
          <div className="mb-6">
            ...
            <h2 className="mt-2 max-w-lg font-display text-3xl font-extrabold leading-tight text-foreground sm:text-4xl">
              Seu patrimônio começa com uma boa decisão.
            </h2>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span
                      className={`mt-1 block text-xs leading-relaxed sm:text-sm ${link.featured ? "text-primary-foreground/80" : "text-muted-foreground"}`}
                    >
                      {link.description}
                    </span>
                    <span
                      className={`mt-2 block text-xs font-bold uppercase ${link.featured ? "text-primary-foreground" : "text-brand-bright"}`}
                    >
                      {link.label}
                    </span>
                  </span>
          ...
          <div className="mt-6 overflow-hidden rounded-md border border-border">
            <img
              src={bannerSrc}
              alt="O consórcio que mais cresce no Brasil — Robson Iza"
              className="block w-full object-cover"
            />
          </div>

          <p className="mt-7 text-center text-xs text-muted-foreground">
            © 2026 Robson Iza · Todos os direitos reservados
          </p>
        </section>
      </div>
    </main>
